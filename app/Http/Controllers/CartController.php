...
class CartController extends Controller
{
    ...
    // protected CouponService       $couponService; // حذف شد: CouponService به صورت مستقیم توسط کنترلر استفاده نمی‌شود
     ...
     * @param CouponService $couponService // نگه داشته می‌شود، اما استفاده مستقیم کاهش می‌یابد
     */
    public function __construct(CartServiceInterface $cartService, CouponService $couponService)
    {
        $this->cartService = $cartService;
        // $this->couponService = $couponService; // حذف شد
    }
    ...
    public function getContents(Request $request): \Illuminate\Http\JsonResponse
    {
        try {
            ...
            if (!$cart || $cart->items->isEmpty()) {
                ...
                ], 200); // 200 چون سبد خالی بودن خطا نیست
            }
    ...
    /**
     * متد کمکی خصوصی برای محاسبه مجموع سبد خرید.
     * Private helper method to calculate cart totals.
     *
    ...
    private function calculateTotals($cart)
    {
        ...
        foreach ($cart->items as $item) {
            $price = $item->productVariant ? $item->productVariant->price : ($item->product ? $item->product->price : 0);
