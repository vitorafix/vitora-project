<?php
// File: app/Services/Managers/CartCacheManager.php (این کامنت به اینجا منتقل شد)
...
class CartCacheManager
{
     ...
     * @param Cart $cart The cart object to store.
     * @return void
     */
    public function put(string $key, Cart $cart): void
    {
        try {
            Cache::put($key, $cart, $this->cacheTtl);
        } catch (\Exception $e) {
            Log::error('Failed to put cart in cache: ' . $e->getMessage(), ['key' => $key, 'cart_id' => $cart->id]);
        }
    }
     ...
     *
     * @param string $key The cache key.
     * @return Cart|null The cart object if found, null otherwise.
     */
     ...
     * @return void
     */
    public function clearCache(?User $user = null, ?string $sessionId = null): void
    {
        try {
            $key = $this->getCacheKey($user, $sessionId);
            Cache::forget($key);
            Log::info('Cart cache cleared', ['key' => $key]);
        } catch (\Exception $e) {
    ...
    /**
     * Retrieves data from cache or executes a callback and stores the result.
     * داده‌ها را از کش بازیابی می‌کند یا یک کال‌بک را اجرا کرده و نتیجه را ذخیره می‌کند.
     *
     * @param string $key The cache key.
