...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function validateFormFields(data) {
        ...
        if (selectedAddressId === 'new' || document.querySelectorAll('input[name="selected_address_id"]').length === 0) {
            ...
            // If an existing address is selected, these fields are not directly required for submission
            // but we should ensure the selected_address_id is present.
        ...
        // Clear live region if all valid, otherwise update with a general error message
        if (formErrorsLiveRegion) {
            if (isValid) {
                formErrorsLiveRegion.textContent = '';
            } else {
                formErrorsLiveRegion.textContent = 'لطفاً تمام فیلدهای الزامی را به درستی پر کنید.'; // Please fill in all required fields correctly.
            }
        }
    ...
    function handleServerValidationError(errors) {
        let errorMessage = 'لطفاً اطلاعات ورودی را بررسی کنید: <br>'; // Please check the input information:
    ...
    function handleClientError(result, status) {
        window.showMessage(result.message || 'خطا در ثبت سفارش. لطفاً دوباره تلاش کنید.', 'error'); // Error placing order. Please try again.
    ...
    function handleRetryableError(result, status, attempt, maxRetries) {
        ...
        if (attempt < maxRetries) {
            window.showMessage('خطا در ثبت سفارش. تلاش مجدد...', 'info', 2000); // Error placing order. Retrying...
        } else {
            window.showMessage(result.message || 'خطا در ثبت سفارش. لطفاً دوباره تلاش کنید.', 'error'); // Error placing order. Please try again.
    ...
    function populateAddressFields(address) {
        addressFields.first_name.value = address.first_name || '';
        addressFields.last_name.value = address.last_name || '';
        addressFields.phone_number.value = address.phone_number || '';
        addressFields.address.value = address.address || '';
        addressFields.province.value = address.province || '';
        ...
        Object.keys(addressFields).forEach(fieldId => { // Iterate over addressFields keys
            const inputElement = addressFields[fieldId]; // Get the actual input element
            if (inputElement) clearFieldError(inputElement);
        });
    }
    ...
    function clearAddressFields() {
        addressFields.first_name.value = '';
        addressFields.last_name.value = '';
        addressFields.phone_number.value = '';
        addressFields.address.value = '';
        addressFields.province.value = '';
    ...
    // Event listener for address radio buttons
    ...
    // Initial load of draft on DOMContentLoaded
    loadDraft();
    ...
    window.addEventListener('beforeunload', function(e) {
        // If there's a draft in Local Storage, it means the user has made changes.
        const hasUnsavedData = localStorage.getItem('orderDraft');
        if (hasUnsavedData) {
            e.preventDefault();
            e.returnValue = 'تغییرات شما ذخیره نشده است. مطمئن هستید که می‌خواهید صفحه را ترک کنید؟'; // Your changes are not saved. Are you sure you want to leave the page?
        }
    });
